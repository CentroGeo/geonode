...
$('.topic .fa-trash').on('click', function() {
  $(this).closest('a').addClass('staged');
  $('#confirmTopic').show();
});

$('#topicConfirm').on('click', function() {
    var mvid = $('.staged').attr('data-mvid');
    var topid = $('.staged').attr('data-topid'); 

     $.ajax({
     url: '/mviewer/topic_remove/'+mvid+'/'+topid,
     type: 'GET',
     data: {},
     dataType: 'html',
     success: function(result) {
