...
def ms_index(request, msurlname=None, template='ms_index.html'):
    ...
    return render(request, template,
                {'config': config, 'sections': sections, 'categorys': categorys,
                 'cat_not_parent': cat_not_parent, 'cat': cat,
...
def ms_section(request, msurlname=None, idsection=None, template='ms_section.html'):
    ...
    return render_to_response(template, RequestContext(request, {'config': config, 'sections': sections, 'config_section': config_section}))
...
def ms_category(request, msurlname=None, idcategory=None, template='ms_category.html'):
    ...
    return render_to_response(template, RequestContext(request, {'config': config, 'sections': sections, 'config_category': config_category, 'categorys': categorys, 'narratives': narratives, 'narratives_category':narratives_category}))
...
def ms_narrative(request, msurlname=None, idnarrative=None, template='ms_narrative.html'):
    ...
    return render_to_response(template, RequestContext(request, {'config': config, 'sections': sections, 'narratives': narratives, 'categorys':categorys, 'config_narrative': config_narrative}))
...
def category_json(request, idms=None, idcategory=None, template='index_vis/vis_category.json'):
    ...
    return render_to_response(template, RequestContext(request, {'category_list': data, 'microsite': microsite}))
...
def ms_json(request, idms=None, template='index_vis/vis.json'):
    ...
    data = list(i for i in category_list)
    print(data)
    return render_to_response(template, RequestContext(request, {'category_list': data, 'narratives': narratives,
                                                                 'microsite': microsite}))
...
def ms_json_admin(request, idms=None, template='index_vis/vis.json'):
    ...
    category_list = Category.objects.filter(microsite=microsite).filter(parent_category__isnull=True)
    data = list(i for i in category_list)
    return render_to_response(template, RequestContext(request, {'category_list': data, 'narratives': narratives, 'microsite': microsite}))
...
def ms_nar_json(request, idms=None, template='index_vis/vis_narratives.json'):
    ...
    return render_to_response(template, RequestContext(request, {'narratives': narratives, 'microsite': microsite}))
...
def ms_gestor_list(request):
    ...
    return render(request, 'ms_gestor_list.html',{'items':ms, 'section': sect,'site':site})


def ms_public_list(request):
    ms = Microsite.objects.filter(public=True)

    return render(request, 'ms_public_list.html', {'items': ms})
...
def upload_ms(request):
    ...
    return render(request, 'microSite_form.html', {'form': ms_form, 'profile':profile, 'no_group': no_group})


@login_required
def ms_category_list(request, ms_id):
    sect = Category.objects.filter(microsite = ms_id)
    return render(request, 'category_list.html',{'items':sect, 'ms_id':ms_id})
...
def upload_section(request, ms_id):
    if request.method == 'POST':
        ...
        ms_form = SectionForm()
    return render(request, 'section_form.html', {'form': ms_form, 'ms_id':ms_id})
...
def update_ms(request, ms_id):
    ...
    if request.method == 'POST':
        if ms_form.is_valid():
            ms_form.save()
            return HttpResponseRedirect('../')
    return render(request, 'microSite_form.html',{'form':ms_form, 'profile':profile, 'obj':obj})
...
def advanced_ms(request, ms_id):
    ...
    if request.method == 'POST':
        if ms_form.is_valid():
            ms_form.save()
            return HttpResponseRedirect('../')
    return render(request, 'microSite_form.html',{'form':ms_form})
...
def ms_update_section(request, ms_id, section_id):
    ...
    if request.method == 'POST':
        if ms_form.is_valid():
            ms_form.save()
            return HttpResponseRedirect('../../ms_detail/'+ms_id)
    return render(request, 'section_form.html',{'form':ms_form})
...
def ms_upload_category(request, ms_id):
    ...
    return render(request, 'category_form.html', {'form': ms_form, 'cat_of':ms_name.name})


@login_required
def ms_narrative_list(request, ms_id):
    narratives_ms = Narrative.objects.filter(microsite = ms_id)
    return render(request, 'narrative_list.html',
                              {
                                  # 'items': category_ms,
                                  'narratives': narratives_ms,
                                  'ms_id': ms_id
                              })
...
def ms_upload_narrative(request, ms_id):
    ...
    return render(request, 'narrative_form.html', {'form': ms_form})
...
def ms_update_category(request, category_id, ms_id):
    ...
    if request.method == 'POST':
        ...
        if ms_form.is_valid():
            ms_form.save()
            return HttpResponseRedirect('../../ms_detail/'+ms_id)
    return render(request, 'category_form.html',{'form':ms_form})
...
def ms_narrative_detail(request, narrative_id, ms_id):
    ...
    return render(request, 'narrative_detail.html',{'narrative':narr, 'ms_id':ms_id})
...
def ms_narrative_publish(request, narrative_id, ms_id):
    ...
    if ms_form.is_valid():
        ms_form.save()
    return render(request, 'narrative_detail.html',{'narrative':obj, 'ms_id':ms_id})
...
def ms_detail(request, ms_id):
    ...
    return render(request, 'ms_detail.html', {
        'cat_of_narras': data,
        'ms': ms,
        'section': sect,
        'site': site,
...
def ms_category_edit(request, ms_id, cat_id):
    ...
    return render(request, 'ms_category_edit.html', {
...
def ms_upload_category_to_parent_cat(request, ms_id, category_id):
    if request.method == 'POST':
        ...
        cat_name = get_object_or_404(Category, id=category_id)
        ms_form = Category_to_ms_Form()
    return render(request, 'category_form.html', {'form': ms_form, 'cat_of':cat_name.name})

@login_required
def ms_update_category_to_ms(request, category_id, ms_id):
    obj = get_object_or_404(Category, id=category_id)
    ms_name = get_object_or_404(Microsite, id=ms_id)
    ms_form = Category_to_ms_Upd(request.POST or None, request.FILES or None, instance=obj)
    if request.method == 'POST':
        if ms_form.is_valid():
            ms_form.save()
            return HttpResponseRedirect('../../ms_detail/'+ms_id)
...
def ms_upload_narrative_to_ms(request, ms_id):
    ...
    return render(request, 'narrative_form.html', {'form': ms_form, 'nar_of':ms_name.name})
...
def ms_list_maps(request):
    ...
    return render(request, 'maps_list.html', {'maps': maps})
...
def ms_list_documents(request):
    ...
    return render(request, 'documents_list.html',{'categorys':category_dict})
