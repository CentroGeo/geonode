...
$(document).ready(function(){
  var id_map_emb = {{mapid}};
  $("#pie_chart_1").draggable();
  //if(is_embed){
    if (typeof id_map_emb != 'undefined') {
      getConstantsTools(id_map_emb);
    }
  //}
});
...
$(".checkbox").click(function() {
  var id_map = $(this).attr("data-mapid");
  getConstantsTools(id_map);

});
function getConstantsTools(id_map){
  var constant_pie = "PIE_GRAPH_INTERSECTION";
  $.ajax({
      data: {'mapid': JSON.stringify(parseInt(id_map)),
           csrfmiddlewaretoken: '{{ csrf_token }}'
          },
      url: '{% url "get_constants_tools_map" %}',
      type: 'POST',
      success : function(data) {
        $('#graphic-visible').hide();
        for (var i = 0; i < data.length; i++) {
          if(constant_pie==data[i]){
...
$('.fa-pie-chart').on("click", function(){
  ...
  $.ajax({
 data: {'layers_base': JSON.stringify(analisys_layer_base),
        csrfmiddlewaretoken: '{{ csrf_token }}'
       },
 url: '{% url "search_base_polygons" %}',
 type: 'POST',
 success : function(data) {
              $('.check-data-general-grafic-pie').html("");
              check = "";
              ...
              layers_count = data[0];
              // titles_layers = data[2];
              // shape_title_markers = data[4];
              // shape_title_base = data[3];

               $('.check-data-general-grafic-pie').html("");
               for (var i = 0; i < analisys_layer_base.length; i++) {

                 check = '<input type="checkbox" name="base_check" value='+analisys_layer_base[i]+'> '+analisys_layer_base_name[i]+'<br>';
...
function second_part_analysis_pie(){
  ...
  arr_obj_bbxylayer["layers"].push([global_layers]);
  ...
  for (var i = 0; i < global_layers.length; i++) {
    if ("No background" != global_layers[i].name){
        if (obj_l.length==0) {
...
$('#tools_analisys').modal('toggle');
                ...
                if(data.length>=1){
                    drawPie("Pie1", data, "#pie_chart_1 .chart", "colorScale20c", 10, 100, 5, 0);
                }
                else{
...
$('.check-data-general-grafic-pie').on("click","input.chb", function(e){
     $('.check-analisys-data-general-grafic-pie').html("");
      for (var i = 0; i < layers_base_analisis.length; i++) {
        radios = "";
        radios = '<input type="checkbox" name="chb" value='+layers_base_analisis[i]+'> '+layers_base_analisis[i]+'<br>';
...
$("#close-general-grafic").on("click", function() {
  $("#pie_chart_1").css("display", "none");
    });
/*End Close the general grafic pie */
/*Update the general grafic pie */
$("#update-general-grafic").on("click", function() {
    update_general_pie();
});
           ...
           $('.check-analisys-data-general-grafic-pie').html("");
             ...
             $('.check-data-general-grafic-pie').html("");
              for (var i = 0; i < layers_count.length; i++) {
                radio = "";
                radio = '<input type="radio" name="base" class="chb" value='+layers_count[i]+'> '+layers_count[i]+'<br>';
 ...
 $(document).ajaxStart(function(){
    ...
    success : function(data) {
                 ...
                 $('#tools_analisys').modal('toggle');
                   ...
                   if(data_pie.length>=1){
                       drawPie("Pie1", data_pie, "#pie_chart_1 .chart", "colorScale20c", 10, 100, 5, 0);
                   }
                   else{
                     $('.less-selected').html("No se puede hacer este conteo debido a que no hay capas o las capas son muy pocas para ser contabilizadas");
                     $("#pie_chart_1").css("display", "none");
   ...
   error : function(message) {
    ...
    // Color Scale Handling...
      ...
      switch (colors)
      {
        case "colorScale10":
          ...
          break;
        case "colorScale20":analisys_layer_base
        case "colorScale20b":
    ...
    // Define an arc generator. This will create <path> elements for using arc data.
      ...
      var tip = d3.tip()
          ...
          .offset([130, 0])
          .html(function(d) {
            return "<span style='color:#B5B4F0'>" + d.data.legendLabel + ", <span>"+d.data.magnitude;
          })
   ...
   // });
      ...
      //       .on("mouseout", synchronizedMouseOut);
            ...
            //Drag.init(document.getElementById("pie_chart_1"));
    ...
    function get_coordinates_by_name_layer(name_layer){
       ...
       url: '{% url "get_geoJson_by_name_layer" %}',
       type: 'POST',
