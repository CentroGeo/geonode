                  ...
                  success: function(result) {
                      if ('values' in result) {
                        ...
                        for(i=0; i < values.length-1;i++){
                            ...
                            + values[i].toFixed(6) + ' - '+ values[i+1].toFixed(6) +'" readonly></td></tr>';
                        }
                        element += '<tr class="styleData rasterNoval"><td><input type="checkbox" class="clasify-row" value="null" style="margin:5px;"></td><td><svg width="15" height="15" style="margin-top:5px;">'
                        + '<rect width="15" height="15" fill="#ffff"/><line x1="0" x2="15" y1="0" y2="15" stroke="red" stroke-width="2"/><line x1="15" x2="0" y1="0" y2="15" stroke="red" stroke-width="2"/>'
                        + '</td><td style="font-size:12px;">Valores nulos.</td><td></td></tr>';
...
$("#styleSave").click(function() {
    ...
    if(data_attr == 'null'){
        ...
        if (type == "symbol"){
            ...
            var styleData = [];
            $(".styleData").each(function(i) {
              if($(this).hasClass('rasterNoval')) {
                styleData.push([null, ['noVal', 'noVal'], 'No value', null, null]);
              } else {
                var sym = $(this).find(".symbol");
                var sfill = sym.attr('fill');
                var sfill_opacity = sym.attr('fill-opacity');
...
$("#clasify-data").on('dblclick','.editable', function() {
    ...
    $(".stvalue1").addClass("editable");
    $(".stvalue2").addClass("editable");
          ...
          if (data[0].unique) {
              ...
              if (data[0].is_raster) {
                ...
                $.each(data[1], function(i, val) {
                  if(val.legend!='no value') {
                    if (!('fill-opacity' in val)){
                      val['fill-opacity'] = '1.0'
                    }

                    stroke_str = '';
                    var min_max = val.legend.split(" ");
                    element += '<tr class="styleData"><td><input type="checkbox" class="clasify-row" style="margin:5px;"></td>'
                    ...
                    + '<title>Doble clic para editar</title></rect>'
                    + '</td><td><input type="text" class="form-control editable input-sm stvalue1" value="'+val.quantity+'" readonly>'
                    + '<label class="hyphen">-</label><input type="text" class="form-control editable input-sm stvalue2" value="'+min_max[2]+'" readonly>'
                    + '</td><td><input type = "text" class="form-control editable input-sm stlgd" value="'
                ...
                if(noVal) {
                    $("#num-classes").val(data[1].length-1);
                } else {
                    $("#num-classes").val(data[1].length);
                }
                $("#data-attr option[value='null']").attr('selected', 'selected');
              } else if ( !data[0].not_cat ) {
                  if (geom_type=='ST_MultiPolygon') {
