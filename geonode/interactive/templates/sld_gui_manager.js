...
function getBands() {
    ...
    $.ajax({
	    ...
	    success: function(data) {
            if (data.type == 'RGBA') {
                var pcontent = '<label for="data-attr">Bandas</label><select class="form-control" id="data-attr">'
	            + '<option value="null">Seleccione</option>';
                $.each(data.limits, function(i, val) {
                    pcontent += '<option value="'+(i+1)+'"> Banda '+(i+1)+' ('+val[0]+'-'+val[1]+')</option>';
                });
                pcontent += '</select>';
...
function data_calculator(idLayer, attr, dataType, rampa, classes) {
	$.ajax({
	    url: '{% url "data_calculator" %}',
	    type: 'POST',
	    ...
	    dataType: 'json',
	    success: function(data) {
	    	// $.each(data.attr_desc, function(index, val) {
	    	// 	pcontent += '<option name="'+index+'">'+index+': '+val+'</option>';
	     //    });
	     //    pcontent += '</select>';
      //       $( "#style-attr" ).html(pcontent);
	    },
