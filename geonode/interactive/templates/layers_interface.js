<script type="text/javascript">
  ...
  var layer_opacity = [];
  var index = [];
  var mapBase = CartoDB_DarkMatter;
  ...
  $('input[name=baseLayers]:radio').filter('[value="Negro"]').attr('checked', 'checked');
  ...
  $('.layerList').on('change', '.checkbox', function() {
    ...
    if(this.checked) {
        ...
        imglegend = $(this).attr('href')+'?request=GetLegendGraphic&format=image%2Fpng&width=20&height=20&layer='+this.value+'&transparent=true&LEGEND_OPTIONS=fontColor:ffffff;fontSize:11;" title="'+this.title;
        ...
        + '<div class="slider"><div class="cslide"><div id="'+dataId+'s" title="'+this.value+'" data-checkid="'+checkId+'" data-id="l'+dataId+'"></div></div><span>100%</span></div></div>';
  ...
  if(rlegendId.indexOf('extLay')>-1) {$("#styleCreate").hide();}
  else {
...
var sentLayer;
$('.layerSentinel').on('click', function() {
    if(!$(this).find('.icon').hasClass("on")) {
        var layerName = $(this).attr('data-layer');

        $(".layerSentinel").find('.icon').removeClass("on");
        $(this).find('.icon').addClass("on");
        if(!jQuery.isEmptyObject(sentLayer)){
            map.removeLayer(sentLayer);
        }
        addSentinelLayer(layerName);
        $("#applyDiv").show();
    }
});

$("#applyParams").click( function() {
    var layerName = $(".layerSentinel.active").attr('data-layer');
    map.removeLayer(sentLayer);
    addSentinelLayer(layerName);
});

function addSentinelLayer(layerName) {
    var initDate = $("#initDate").val();
    var endDate = $("#endDate").val();
    var cCover = $("#sliderSentinel").attr("data-value");
    var baseUrl = "https://services.sentinel-hub.com/ogc/wms/3263b609-c862-4d92-a166-babfde1ee375";

    if(initDate=='') {
        var d = new Date();
        var month = d.getMonth();
        var day = d.getDate();
        initDate = d.getFullYear() + '-' + ((''+month).length<2 ? '0' : '') + month + '-' + ((''+day).length<2 ? '0' : '') + day;
    }
    if(endDate=='') {
        var d = new Date();
        var month = d.getMonth()+1;
        var day = d.getDate();
        endDate = d.getFullYear() + '-' + ((''+month).length<2 ? '0' : '') + month + '-' + ((''+day).length<2 ? '0' : '') + day;
    }

    sentLayer = L.tileLayer.betterWms(baseUrl, {
        tileSize: 512,
        urlProcessingApi:"https://services.sentinel-hub.com/ogc/wms/0f7ce419-0d50-44d8-9ca1-bbb3a5b34888",
        maxcc:cCover,
 	 	//minZoom:6,
 	 	//maxZoom:16,
 	 	preset:layerName,
 	 	layers:layerName,
 	 	time:initDate+"/"+endDate,
        attribution: '&copy; <a href="http://www.sentinel-hub.com/" target="_blank">Sentinel Hub</a>'
      }).addTo(map);
}

/* Slider sentinel */
$("#sliderSentinel").mousedown(function(){
	var max = $("#trackSentinel").width();
	var handle = $(this);
	function handleMove(e){
		var l = Math.max(3,3+Math.min(e.pageX - $("#trackSentinel").offset().left,max));
		handle.css("left",l);
		$("#sliderSentinel").attr('data-value',l/100);
		$("#cloudCover").text(l+'%');
	};
	function handleUp(){
