...
class AlfTable(models.Model):
    ...
    filter_column = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name='alf_cfilter_attribute',
        on_delete=models.CASCADE)

    def __unicode__(self):
        return self.table_name

    def remove_alftable(self):
...
class AlfTableAttribute(models.Model):
    """
        Auxiliary model for storing ALF table attributes.
    """
    alf_table = models.ForeignKey(
        AlfTable,
        blank=True,
        null=True,
        unique=False,
...
class AlfLayer(ResourceBase):
    """
    Modelo para capas del censo Agropecuario 2007
    """
    alf_table = models.ForeignKey(AlfTable, null=True, blank=True, on_delete=models.CASCADE)
...
def pre_delete_alftable(instance, sender, **kwargs):
    instance.remove_alftable()


def pre_delete_alflayer(instance, sender, **kwargs):
    instance.remove_alflayer()


signals.pre_delete.connect(pre_delete_alftable, sender=AlfTable)
signals.pre_delete.connect(pre_delete_alflayer, sender=AlfLayer)